...
export function ServiceCard({ label, description, onOpen }: ServiceMetadata & { onOpen: () => void }) {
  return (
    <article className="service-card">
      <p className="eyebrow">Microfrontend</p>
      <h2>{label}</h2>
      <p>{description}</p>
      <button type="button" onClick={onOpen}>Open service</button>
    </article>
  )
}
...
export function ServiceRemotePage({
  ...
  return (
    <section className="service-remote">
      ...
      {onNavigateHome && <button type="button" onClick={onNavigateHome}>Return to Relay home</button>}
    </section>
  )
}
